...
This a Clojure nREPL client written in Lua. It implements almost all OP codes of Clojure nREPL messages(except for ":load-file" & ":interrupt").

Please be aware of this implementation use nREPL's default encoding "bencode" to encode/decode messages between client and nREPL server.

0. 	PREPARE:
================
have your nREPL server started; make sure files clj_client.lua & bencode.lua under your lua path.
...
2.  PLAY:
================
When you see a prompt like "user=>: ", you can now try typing some Clojure code to send(e.g. "*ns*").
User input starting with an exclamation mark(!) means special command. Special commands are usually special nREPL OPerations(e.g. "!session" for ":sessions" OP), or configuration command(e.g. "!timeout 0.1" is to set timeout to 0.1 second), or, to make it's more interesting, I just had it also kind of Lua code executable under "!". 
To sum up:
